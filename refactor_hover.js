...
code = code.substring(0, modalStart) + 'const IdentityHoverCard = ({ userId, username, position }) => {' + code.substring(code.indexOf('{', modalStart) + 1);

code = code.replace(/const cardRef = useRef\(null\);\n  const \[isExporting, setIsExporting\] = useState\(false\);/, '');
code = code.replace(/const handleExport = async \(\) => \{.*?\};\n/s, '');

let returnPortalRegex = /return createPortal\(\s*<AnimatePresence>.*?<\/AnimatePresence>,\s*document\.body\s*\);/s;

let newPortalContent = `
  return (
    <motion.div
...
code = code.replace(returnPortalRegex, newPortalContent);

code = code.replace(/const \[showModal, setShowModal\] = useState\(false\);\n/, '');
code = code.replace(/const handleClick = useCallback\(\(e\) => \{.*?\}, \[\]\);/s, 'const navigate = useNavigate();\n  const handleClick = useCallback((e) => { e.preventDefault(); e.stopPropagation(); navigate(`/profile/${username}`); }, [navigate, username]);');
code = code.replace(/const tooltipWidth = 288;/g, 'const tooltipWidth = 600;');
code = code.replace(/const tooltipHeight = 200;/g, 'const tooltipHeight = 420;');
code = code.replace(/\(showTooltip \|\| showModal\)/g, 'showTooltip');
code = code.replace(/<HoverTooltip data=\{profileQ\.data\} position=\{tooltipPos\} onExpand=\{handleClick\} \/>/, '<IdentityHoverCard userId={userId} username={username} position={tooltipPos} />');
code = code.replace(/\{showModal && \([\s\S]*?\}\)/s, '');
code = code.replace(/&& !showModal/, '');

fs.writeFileSync('client/src/components/MemberHoverCard.jsx', code);
